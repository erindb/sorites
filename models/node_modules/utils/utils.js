...
function shift_dist(erp, eps, widths) {
  ...
  // P(a <= x - eps < b) = P(a+eps <= x < b+eps)
  // To determine the NEW probability mass between a and b,
  // Find the OLD probability mass between a+eps and b+eps

  var lowers = []; //a
  var uppers = []; //b
  var old_lowers = []; //a+eps
  var old_uppers = []; //b+eps
  for (var i=0; i<vals.length; i++) {
    ...
    lowers.push(a);
    uppers.push(b);
    old_lowers.push(a+eps);
    old_uppers.push(b+eps);
  }
  // The OLD probability mass between a+eps and b+eps ignores all probability
  // mass below eps. So change the first lower-bound (a+eps) to 0 instead.
  old_lowers[0] = 0;
  ...
  for (var i=0; i<vals.length; i++) {
    var new_a = lowers[i];
    var new_b = uppers[i];
    var old_a = old_lowers[i];
    var old_b = old_uppers[i];
    // find the probability mass between a+eps and b+eps
    var new_prob = 0;
    for (var j=0; j<vals.length; j++) {
      ...
      var p = old_probs[j];
      if (b <= old_a) {
        // ignore any bins that end before the region of interest
      } else if (a <= old_a && old_a <= b && b <= old_b) {
        // if the bin starts before and ends within the region of interest,
        // just take the proportion of THIS BIN that overlaps with the region of
        // interest.
        var new_proportion = (b - old_a) / (b - a);
        new_prob += p*new_proportion;
      } else if (a <= old_a && old_b <= b) {
        // if the region of interest is fully inside this bin, take the
        // proportion of the THIS BIN that overlaps with the region of interest.
        var new_proportion = (old_b - old_a) / (b - a);
        new_prob += p*new_proportion;
      } else if (old_a <= a && b <= old_b) {
        // if this bin is fully inside the region of interest, take the full
        // probability of the bin.
        new_prob += p;
      } else if (old_a <= a && a <= old_b && old_b <= b) {
        // if this bin starts after the region of interest and ends after the
        // region of interest, take the proportion of THIS BIN that overlaps
        // with the region of interest
        var new_proportion = (old_b - a) / (b - a);
        new_prob += p*new_proportion;
      }
    }
    new_probs.push(new_prob);
  }

  // console.log(eps);
  // console.log(new_probs);
  // console.log(_.sum(new_probs));
  assert(Math.abs(_.sum(new_probs) - 1) < 0.000001);
