...
function even_bins(dollar_amounts) {
  ...
  for (var i=0; i<n_bins; i++) {
    ...
    if (cutoff_below<next_dollar_amount && next_dollar_amount<cutoff_above) {
      ...
      width_lookups[mid] = (cutoff_above - cutoff_below);
...
function bins(dollar_amounts) {
  ...
  for (var i = 0; i < dollar_amounts.length; i++) {
    ...
    width_lookups[current_midpoint] = (cutoff_above - cutoff_below);
...
function shift_dist(erp, eps, widths) {
  var erp = JSON.parse(erp);
  var vals = erp["support"];
  var old_probs = erp.probs;

  // Keep the same bins.
  // Change the probabilities so that it's P(x -eps \in bin), not P(x \in bin).
  // For each bin, go up eps through the support to collect new probability
  // mass from above. Keep track of where that probability mass came from and
  // remember to subtract it later.

  var prob_mass_to_add_from_above = {};
  var prob_mass_to_subtract_from_below = {};
  for (var i = 0; i < vals.length; i++) {
    prob_mass_to_add_from_above[i] = 0;
    prob_mass_to_subtract_from_below[i] = 0;
  }

  // For each bin,
  for (var i = 0; i < vals.length; i++) {
    var bin_mid = vals[i];
    var bin_width = widths[i];
    var bin_prob = widths[i];
    // Go up eps through the support to collect new probability mass from above.
    var rest_of_eps = eps;
    var j = i;
    // If we haven't yet reached eps, keep looking at higher bins.
    while (j < vals.length && rest_of_eps > 0) {
      var higher_mid = vals[j];
      var higher_width = vals[j];
      var higher_prob = vals[j];
      // Go up eps (or however far we can go)
      var partial_eps = Math.min(rest_of_eps, higher_width);

      // Collect new probability mass from above
      var prop_of_bin_to_collect = partial_eps/higher_width;
      var prob_to_transfer = prop_of_bin_to_collect*higher_prob;
      prob_mass_to_add_from_above[i] += prob_to_transfer;
      // Keep track of where that probability mass came from and remember to
      // subtract it later.
      prob_mass_to_subtract_from_below[j] += prob_to_transfer;

      // Keep track of how much farther we need to go to get to eps.
      var rest_of_eps = rest_of_eps - partial_eps;
    }
  }

  console.log(prob_mass_to_add_from_above);
  console.log(prob_mass_to_subtract_from_below);

  var new_probs = [];
  for (var i = 0; i < vals.length; i++) {
    new_probs.push(old_probs[i] + prob_mass_to_add_from_above[i] - prob_mass_to_subtract_from_below[i]);
  }

  // console.log(new_probs);
  // console.log(_.sum(new_probs));

  return {ps: new_probs, vs: vals};
};
