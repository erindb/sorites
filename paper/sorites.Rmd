...
```{r sorites cor cluster}
# # shows two outliers
# (1 - pairwise_correlation_matrix_sorites) %>% dist() %>% hclust() %>% plot()
```

```{r sorites pairwise cors}
pairwise_correlation_matrix_sorites = df11 %>%
  mutate(qid = paste(qtype, object, dollar_amount)) %>%
  select(workerid, qid, response) %>%
  spread(qid, response) %>%
  select(-workerid) %>%
  t() %>%
  cor(method="spearman")
different_workers = c(10, 15)
get_pairwise_cors_sorites = function(df) {
  pairwise_correlation_matrix_sorites = df %>%
    mutate(qid = paste(qtype, object, dollar_amount)) %>%
    select(workerid, qid, response) %>%
    spread(qid, response) %>%
    select(-workerid) %>%
    t() %>%
    cor(method="spearman")
  pairwise_cors_sorites = pairwise_correlation_matrix_sorites %>%
    .[upper.tri(., diag=F)]
  pairwise_correlation_range_sorites = pairwise_cors_sorites %>%
    quantile(probs = c(0, 1)) %>%
    round(3) %>%
    unname()
  pairwise_correlation_mean_sorites = mean(pairwise_cors_sorites) %>% round(3)
  return(list(mean = pairwise_correlation_mean_sorites, range = pairwise_correlation_range_sorites))
}
pairwise_sorites_majority = get_pairwise_cors_sorites(df11 %>% filter(!(workerid %in% different_workers)))
df_minority = df11 %>% filter(workerid %in% different_workers)
minority_cor = df_minority %>%
  get_pairwise_cors_sorites()
mag_vs_min_cors = pairwise_correlation_matrix_sorites[different_workers,-different_workers] %>% .[upper.tri(., diag=F)]
maj_vs_min = list(mean = mean(mag_vs_min_cors) %>% round(3), range = mag_vs_min_cors %>%
    quantile(probs = c(0, 1)) %>%
    round(3) %>%
    unname())
pairwise_sorites_majority_inductive = df11 %>%
  # filter(!(workerid %in% different_workers)) %>%
  filter(qtype == "inductive") %>%
  get_pairwise_cors_sorites()
pairwise_sorites_majority_concrete = df11 %>%
  # filter(!(workerid %in% different_workers)) %>%
...
split_half_cor = function(x) {
  half = sample(
    all_workers,
    size = length(all_workers)/2,
    replace = F
  )
  df11 %>% mutate(half = ifelse(workerid %in% half, "A", "B")) %>%
...
n_resamples = 1000
resamples = sapply(1:n_resamples, split_half_cor)
split_half_conf = quantile(resamples, probs = c(0.025, 0.975)) %>% round(3) %>% unname()
split_half_correlation = mean(resamples) %>% round(3)
```

Participants tended to be in agreement about their responses. 26 of the 28 participants had only positive pairwise correlations with one another (average Spearman correlation: `r pairwise_sorites_majority$mean`, range: [`r pairwise_sorites_majority$range[1]`, `r pairwise_sorites_majority$range[2]`]). The remaining 2 participants showed no clear pattern in their responses and were uncorrelated with one another (Spearman correlation `r minority_cor$mean`) and usually uncorrelated with the other 26 participants (average Spearman correlation: `r maj_vs_min$mean`, range: [`r maj_vs_min$range[1]`, `r maj_vs_min$range[2]`]). The split half correlation of mean responses to each question was `r split_half_correlation` (bootstrapped 95% confidence interval: [`r split_half_conf[1]`, `r split_half_conf[2]`]).

Results of Experiment 1 are shown in Figure 1. We qualitatively capture the three characteristic effects of vagueness:

* We see that judgements depend on the object category (and on the price and statement type).
