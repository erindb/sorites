...
pairwise_correlation_range_sorites = pairwise_cors_sorites %>%
  ...
  round(3) %>%
  unname()
pairwise_correlation_mean_sorites = mean(pairwise_cors_sorites)
```
...
df11 %>% filter(workerid %in% different_workers) %>%
  mutate(workerid = factor(workerid)) %>%
  ggplot(aes(x=dollar_amount, y=response, colour=object, shape=workerid, alpha=trial_number)) +
  # geom_line() +
  geom_point() +
  # geom_text_repel() +
  facet_wrap(qtype~object, ncol=5, scales="free_x") +
  # facet_grid(workerid~qtype) +
  scale_y_continuous(breaks=1:9, limits=c(0.9, 9.1)) +
  xlab("Dollar Amount") +
  ylab("Endorsement Rating")
get_pairwise_cors_sorites(df11 %>% filter(!(workerid %in% different_workers)))
```

Participants tended to be in agreement, though response patterns varied. The pairwise correlations between participants' responses were on average `r pairwise_correlation_mean_sorites` (range: [`r pairwise_correlation_range_sorites[1]`, `r pairwise_correlation_range_sorites[2]`]).
...
<!-- Participants tended to be in agreement. The split half correlation of mean responses to each question was `r split_half_correlation` (bootstrapped 95% confidence interval: [`r split_half_conf[1]`, `r split_half_conf[2]`]). -->
