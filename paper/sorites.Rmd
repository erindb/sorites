...
For the concrete premise, we used an existing quantitative model of scalar adjectives (@Lassiter2013). This is a Rational Speech Act model, which makes use of pragmatic principles {{GRICE}} to resolve vague language in context {{RSA}}. In this model, the literal semantics of a scalar adjective is relative to some unspecified threshold.
...
For the inductive premise, we build off of @Lassiter2013's model of scalar adjectives.
...
1.) Participants imagine that they know of an object X that is expensive, inferring a price $x$ and threshold $\theta$ that would be described as "expensive" under @Lassiter2013's model.
...
$$\begin{aligned}L_0(x | \mbox{still expensive}, \theta) &=
\frac{Pr(x)\delta_{x - \varepsilon > \theta}}{\int_\theta Pr(x)\delta_{x - \varepsilon > \theta} d\theta} \\ &=
\left\{\begin{array}{ll}\frac{Pr(x)}{1 - {CDF}(\theta + \varepsilon)} & \mbox{if } \ x - \varepsilon > \theta \\ 0 & \mbox{otherwise}\end{array}\right\}\end{aligned}$$
...
$$\begin{aligned}S_1(u|x, \theta) = \frac{\exp(\lambda \left( \ln(L_0(x|u, \theta)) - c(u)\right))}{\sum_{u'} \exp(\lambda\left( \ln(L_0(x|u', \theta))-c(u') \right))} = \frac{ C_u \left( L_0(x|u, \theta)\right)^\lambda}{\sum_{u'} C_{u'} \left(L_0(x|u', \theta)\right)^\lambda}\end{aligned}$$

So for the "still expensive" utterance:

$$\begin{aligned}S_1(\mbox{still expensive}|x, \theta) &=
\frac{C_X \left(L_0(x|\mbox{still expensive}, \theta)\right)^\lambda}
{C_X \left(L_0(x|\mbox{still expensive}, \theta)\right)^\lambda + C_S \left(L_0(x|\mbox{silent}, \theta)\right)^\lambda} \\
&= 
\left\{\begin{array}{ll}
\frac{C_X \left(\frac{Pr(x)}{1 - {CDF}(\theta + \varepsilon)}\right)^\lambda}
{C_X \left(\frac{Pr(x)}{1 - {CDF}(\theta + \varepsilon)}\right)^\lambda + C_S \left(Pr(x)\right)^\lambda} & \mbox{if } \ x - \varepsilon > \theta \\ 0 & \mbox{otherwise}\end{array}\right\} \\
&= 
\left\{\begin{array}{ll}
\frac{C_X}{C_X + C_S\left(1 - {CDF}(\theta + \varepsilon)\right)^\lambda} & \mbox{if } \ x - \varepsilon > \theta \\ 0 & \mbox{otherwise}
\end{array}\right\}\end{aligned}$$

4.) Finally, we average over participant's initial guess of $x$ and $\theta$ to get the expected endorsement.

$$\begin{aligned}S_1(\mbox{still expensive}) = \int_{x,\theta} L_1(x,\theta|\mbox{x is expensive})
\left\{\begin{array}{ll}
  \frac{C_X}{C_X + C_S\left(1 - {CDF}(\theta + \varepsilon)\right)^\lambda} & \mbox{if } \ x - \varepsilon > \theta \\
  0  & \text{otherwise.}
\end{array}\right\}
dxd\theta\end{aligned}$$

If the costs of the two utterances are equal, then the $S_1(\mbox{still expensive}|x, \theta)$ value is always between $1/2$ and $1$ whenever $x-\varepsilon > \theta$. In this case, the expected endorsement is very similar to simply oberving the joint distribution $L_0(x, \theta | \mbox{X is expensive})$ and taking the expectation of $x-\varepsilon > \theta$. But the smaller $\theta$ is, the less informative the "still expensive" utterance would be, and so the less likely it is that a speaker would actually endorse it over staying silent.
...
```{r load model fit to priors only}
m12fit = read.csv("../models/results/final_expts_results-S1-sigmax10_10000_burn5000_lag10_chain1_inductive_version_s1_inductive_bins_no_cost_param_listener0_normed_rating_ignore_last_bin.csv",
  col.names = c("result_type", "variable",
                "IGNORE", "object",
                "value", "probability"))
```

```{r reformat full model fit (sorites, but fit only to priors)}
dfm12fit = m12fit %>% filter(result_type %in% c("S1", "Inductive")) %>%
  group_by(result_type, object, variable) %>%
  do(quantile_errorbars(.$value)) %>%
  ungroup() %>%
  gather("region", "value", c(y, ymin, ymax)) %>%
...
```{r fit only to priors, sorites fit figure, fig.width=10, fig.height=4, fig.cap="Results of Experiment 1. Error bars are 95% confidence intervals."}
 breaks_fun <- function(y) {
  if (max(y) < 2) {
    seq(0, 1, 0.2)
  } else {
...
```{r sorites fit cors}
concrete_cor12 = dfm12fit %>% select(-c(ymax, ymin)) %>%
  spread(src, y) %>%
  filter(qtype=="concrete") %>%
  with(cor(Model, `Sorites Experiment`)^2 %>% round(3))
inductive_cor12 = dfm12fit %>% select(-c(ymax, ymin)) %>%
  spread(src, y) %>%
  filter(qtype=="inductive") %>%
  with(cor(Model, `Sorites Experiment`)^2 %>% round(3))
concrete_cor12
