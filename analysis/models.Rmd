...
concrete_model_agg %>%
  ...
  ggplot(., aes(x=model, xmin=model_low, xmax=model_high,
                y=data, ymin=data_low, ymax=data_high)) +
...
df %>% filter(qtype=="concrete") %>% select(object, dollar_amount, response) %>%
  ...
  stat_ecdf(geom="step") +
  facet_wrap(~object, scales="free", ncol=5)
ggsave("images/concrete_fit.png", width=10, height=2)
```

```{r, fig.width=12, fig.height=3}
# df %>% filter(qtype=="concrete") %>%
#   select(object, dollar_amount, response) %>%
#   mutate(src = "data") %>%
#   rbind(., concrete_premise_fit %>%
#           filter(type=="concrete") %>%
#           mutate(response = value, src = "model") %>%
#           select(object, dollar_amount, response, src)) %>%
#   as.data.frame %>%
