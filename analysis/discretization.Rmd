...
```{r}
tf = read.csv("test.csv", header=F, col.names = c("x", "theta", "dir", "wppl_prob")) %>%
  mutate(dir = as.character(ifelse(dir == "theta < x", "theta x", "x theta"))) %>%
  mutate(x = ifelse(x=="(3.75, Infinity)", "(3.75, inf)", as.character(x))) %>%
  mutate(theta = ifelse(theta=="(3.75, Infinity)", "(3.75, inf)", as.character(theta)))
with(
  merge(tf, df %>% mutate(dir = as.character(dir)),
        by=c("x", "theta", "dir")),
  stopifnot(mean(approx(probability, wppl_prob))==1))
```
...
stopifnot(mean(approx(marginal_x$probability, x_probability))==1)
```
