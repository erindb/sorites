...
load_give_a_number = function(re_run = F) {
  if (re_run) {
    give_a_number = read.csv(data_dir("/priors/give-a-number.csv")) %>%
      ...
      select(-item)
    write.csv(give_a_number,
              data_dir("priors/give_a_number_reformatted.csv"),
              row.names = F)
  } else {
    give_a_number = read.csv(
      data_dir("priors/give_a_number_reformatted.csv"))
  }
  return(give_a_number)
}

load_sorites = function(re_run = F) {
  ...
  if (re_run) {
    ...
    write.csv(df, data_dir("sorites/combined_reformatted_sorites_data.csv"), row.names = F)
  } else {
    df = read.csv(data_dir("sorites/combined_reformatted_sorites_data.csv"))
  }
  df = df %>% mutate(
...
load_priors = function(re_run = F) {
  if (re_run) {
    ...
    write.csv(prior_bins_data, cache_dir("prior_bins_data_cache.csv"), row.names = F)
  } else {
    prior_bins_data = read.csv(cache_dir("prior_bins_data_cache.csv"))
  }
