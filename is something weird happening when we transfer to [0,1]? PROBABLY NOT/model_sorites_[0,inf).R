...
#scale to max 1:
examples.grid.ub <- lapply(unscaled.examples, max)
...
listener0 = function(utterance.idx, thetas.idx, degree.idx, pdf, cdf, dist, index) {
  if (index == T) {
    ...
    thetas = thetas.idx
    degree = degree.idx
    if (utterance.idx == 1) { return(non.cached.pdf(degree, dist)) }
    if (utterance.polarities[utterance.idx] == +1) {
      theta = thetas[utterance.idx-1]
      utt.true = degre <= theta
      true.norm = non.cached.cdf(theta, dist)
      return(utt.true*non.cached.pdf(degree, dist)/true.norm)
    }
  }
}

speaker1 = function(thetas.idx, degree.idx, utterance.idx, alpha, utt.cost, pdf, cdf, dist, index) {
  if (index==T) {
    if(is.na(S1.cache[degree.idx,thetas.idx[1],utterance.idx])) {
      ...
      for(i in 1:length(possible.utterances)) {
        l0 = listener0(i, thetas.idx, degree.idx, pdf, cdf, dist, index=T)
    ...
    utt.probs = array(0,dim=c(length(possible.utterances)))
    for(i in 1:length(possible.utterances)) {
      l0 = listener0(i, thetas, degree, pdf, cdf, dist, index=F)
      utt.probs[i] <- (l0^alpha) * exp(-alpha * utt.cost *  utterance.lengths[i])
    }
...
listener1 = function(utterance, alpha, utt.cost, n.samples, step.size,
  ...
  prob.unnormed = function(state) {
    ...
    #check bounds:
    if (any(state < 0)) { return(0) }
    if (any(state > examples.grid.ub[[dist]]) {
      degree = state[1]
      thetas = state[2]
      prior = non.cached.pdf(degree, dist)
      likelihood = speaker1(thetas, degree, utt.idx, alpha, utt.cost, pdf, cdf, dist, index=F)
    ...
    likelihood = speaker1(thetas.idx, degree.idx, utt.idx, alpha, utt.cost, pdf, cdf, dist, index=T)
    return(prior*likelihood)
  }
