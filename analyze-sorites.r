...
get.prior.bins = function(item) {
  ...
  vals = c(vals, seq(vals[length(vals)], maxes[[item]], length.out=10))
  responses = sort(item.data$response)
  #responses = c(responses, 9)
#   plot(vals, responses, main=item)
  responses = c(responses, seq(responses[length(responses)], 9, length.out=10)) #highest expensive rating is 9
#   if (item == "watch") {
#     plot(vals, responses)
#   }
