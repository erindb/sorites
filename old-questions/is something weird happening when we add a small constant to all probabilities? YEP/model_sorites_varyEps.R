...
vary.eps = function(mini.epsilon) {
  ...
  make.pdf.cache <- function(kernel.est) {
    k = kernel.est$y + mini.epsilon#0.00001
    area <- sum(k) 
    normed.dens <- k/area
    return(normed.dens)
  }
  ...
  cols <- c(model.cols)
  png(paste(c("scatterplot_eps=",mini.epsilon,".png"),collapse=""), 1000, 800, pointsize=32)
...
#for (e in c(0.00001, 10^(-6), 10^(-7), 10^(-10), 10^(-12), 10^(-15))) {
for (e in c(10^(-30))) {
  vary.eps(e)
}