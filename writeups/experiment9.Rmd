...
aggrd2c = d2c %>% group_by(domain, item, lowers, condition) %>%
  ...
  rename(normed.responses=mean) %>% as.data.frame
...
aggrd2c.priors %>% gather('source', 'probability', c(5,9)) %>%
  mutate(source = factor(source,
                         levels=c('data.responses', 'lognormal.responses'),
                         labels=c('data', 'lognormal fit')),
         ci.low = ifelse(source=='data', ci.low, NA),
         ci.high = ifelse(source=='data', ci.high, NA)) %>%
