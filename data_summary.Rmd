...
```{r, fig.width=8, fig.height=4}
exp = read.csv("data/priors/data_exp03_2013_12_03_05.csv") %>%
...
```{r, fig.width=8, fig.height=4}
exp = read.csv("data/priors/data_exp04_2013_12_03_12.csv") %>%
  gather("question", "responses", c(Answer.1, Answer.2, Answer.3, Answer.4)) %>%
  filter(workerid != "20")
list_data = mapply(
  function(response, question, workerid, a5, a6, a7, a8, a9) {
...
```{r}
exp = read.csv("data/priors/data_exp05_2013_12_04_15.csv") %>%
  gather("question", "responses", c(Answer.1, Answer.2, Answer.3, Answer.4))
  list_data = mapply(
    function(response, question, workerid, a5, a6, a7, a8, a9) {
      # char(a5) %>% fromJSON # we also collected a max price
  ...
  p = rbind(agg, sim) %>%
    ggplot(., aes(x=bin, y=rating, fill=source)) +
    # geom_point(alpha=1/10) +
    geom_bar(stat="identity", alpha=1/2, position=position_dodge(0.9)) +
    geom_errorbar(aes(ymin=low, ymax=high, colour=source),
                  width=0, position=position_dodge(0.9))+
    facet_wrap(~item, scale="free") +
    scale_fill_brewer(type="qual", palette = 3) +
    scale_colour_brewer(type="qual", palette = 3)
  return(list(plot=p, params=inferred_parameters))
```
