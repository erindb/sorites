...
```{r}
rs %>% filter(type=="prior" & (label=="mu" | label=="sigma")) %>%
...
```{r}
give_a_number %>% ggplot(., aes(x=price)) +
  geom_histogram() +
...
```{r}
# %>%
#   ggplot(., aes(x = sample, colour=object, fill = object)) +
#   geom_histogram(alpha=1/2) +
#   theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
#   geom_histogram(data=(give_a_number %>% mutate(src="data")), aes(x=price, colour=object, fill=object), alpha=opacity) +
...
#   geom_point()
cor(comparison_values$value, comparison_values$response)
all_comparison = merge(rs %>% filter(type!="prior") %>% select(type, object, dollar_amount, value),
