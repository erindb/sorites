...
Model fit for Give a Number and sorites concrete premise. Model results are in black, actual data is in red.
...
rs %>% filter(type=="prior", label=="price") %>%
  ...
  ggplot(., aes(x=value)) +
  geom_density(colour="black", alpha=opacity) +
  facet_grid(~object) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_density(data=give_a_number, aes(x=price), colour="red", alpha=opacity)
...
rs %>% filter(type=="concrete") %>%
  ...
  # scale_colour_brewer(type="qual", palette = 3) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_point(data=agg %>% filter(qtype=="concrete"), aes(x=dollar_amount, y=(response-1)/8),
