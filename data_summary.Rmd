...
agg_bins %>%
  ggplot(., aes(x,y, ymin=low, ymax=high, group=group)) +
  geom_line() +
  geom_ribbon(alpha=1/2) +
  facet_grid(exp~item, scale="free") +
  xlab("bin midpoint") +
  ylab("normalized slider response") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
if (RUN_MODEL) {
  ...
  raw_rs = webppl(
    ...
    write.csv(rs, ".model_cache.csv", row.names = F)
} else {
  rs = read.csv(".model_cache.csv")
}
```
