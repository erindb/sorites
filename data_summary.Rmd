...
```{r}
# df %>% select(dollar_amount, object, response, qtype)
```
...
Ignore this for now, it was an odd task.

```{r}
# exp2 = read.csv("data/priors/data_exp02_2013_10_08_08.csv") %>%
#   gather("var", "val", 30:53)
```
...
if (RUN_DATA_TRANSFORM) {
  ...
  process_bins_data = function(filename) {
    ...
    df = do.call(rbind, list_data) %>%
      ...
      ungroup() %>%
      select(item, bin, UB, LB, rating, condition) %>%
      as.data.frame
...
n_split = nrow(prior_bins_data %>% filter(condition == "split"))
```

In the "split" condition (`r n_split` participants), participants saw one bin at a time. In all other versions, the sliders for each item were shown together.

```{r}
# webppl(program_file = "models/prior_fits/bins.wppl",
#        inference_opts = list(method="incrementalMH", samples=1),
#        model_var = "model",
#        data_var = "ARGS",
#        data = prior_bins_data)
```
...
# analyze_bins_data = function(filename) {
#   exp = read.csv(filename) %>%
#     gather("question", "responses", c(Answer.1, Answer.2, Answer.3, Answer.4))
#   list_data = mapply(
...
#     mutate(UB = num(ifelse(uppers=="infty", max, char(uppers))),
#            LB = num(lowers),
#            width = UB-LB,
#            rating = num(responses)/sum(num(responses))) %>%
#     ungroup()
...
#       unique(df$item),
#       function(current_item) {
#         params = webppl(program_file = "models/prior_fits/bins.wppl",
#                         model_var = "model",
#                         packages = c("models/node_modules/utils",
#                                      "models/node_modules/babyparse",
#                                      "models/node_modules/babyparse"),
#                         inference_opts = list(method="MCMC",
#                                               samples=iterations,
#                                               burn=100,
...
#                         data_var = "ARGS",
#                         data = list(
#                           responses=with(df %>% filter(item==current_item), mapply(
#                             function(lb, ub, r) {list(LB=lb, UB=ub, rating=r)},
#                             LB, UB, rating,
#                             SIMPLIFY=F)))) %>%
#           mutate(item = current_item)
#         return(params)
...
#       source="data")
#   
#   sim = merge(df, inferred_parameters) %>% 
#     group_by(item, mu, sig, UB, LB, bin) %>%
#     mutate(
#       rating = exp(mapply(function(UB, LB, mu, sig) {
#         pnorm(log(UB), mu, sig) - pnorm(log(LB), mu, sig)
#       }, UB, LB, mu, sig)),
#       source="simulated",
#       low = rating,
#       high = rating
...
#   
#   p = rbind(agg, sim) %>%
#     ggplot(., aes(x=bin, y=rating, fill=source, colour=source)) +
#     geom_bar(stat="identity", alpha=1/2, position=position_dodge(0.9)) +
#     geom_errorbar(aes(ymin=low, ymax=high),
#                   width=0, position=position_dodge(0.9))+
#     facet_wrap(~item, scale="free") +
#     scale_fill_brewer(type="qual", palette = 3) +
#     scale_colour_brewer(type="qual", palette = 3)
#   return(list(p=p, inferred_parameters=inferred_parameters))
# }
# output = analyze_bins_data("data/priors/data_exp03_2013_12_03_05.csv")
# print(output[["inferred_parameters"]])
# print(output[["p"]])
# ggsave("exp3.png", width=8, height=4)
```
...
* $logit(p_{ib}) = \int_{LB_{ib}}^{UB_{ib}} \varphi(ln(t)|\mu_i, \sigma_i) dt$

Observations from experimental data:

* $logit(d_{ib}) \sim \mathcal{N}(logit(p_{ib}), \sigma_{binned\ hist})$
