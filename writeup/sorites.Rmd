...
It's not clear what the alternative utterances in the adjective model should be. I use "adjective" and "say nothing" for computational ease when fitting parameters in the simulations. I then run the model, with these best parameter settings, for a few different sets of alternative utterances:
...
```{r echo=F}
### graph responses for expt 1
```

GRAPH RESPONSES

WRITE TO FILE FOR MODEL COMPARISON
...
write.csv(aggregate_prior_distributions[,c("item", "lowers", "normed_responses")],
          "simulations/mean_priors.csv", row.names=F)
write.csv(prior_experiment_data[,c("item", "lowers", "normed_responses", "workerid")],
          "simulations/all_priors.csv", row.names=F)
```
...
write.csv(aggregate_sorites_data, "simulations/aggregate_sorites_data.csv", row.names=F)
```

## Experiment 4: Relative clause version
...
write.csv(aggregate_sorites_relative_data, "simulations/aggregate_sorites_relative_data.csv", row.names=F)
```
...
### Fit and compared to Experiment 1: Original

COMPARE TO EXP1 (SCATTER, LINES)

### Fit and compared to Experiment 3: Conditional
...
The model probabilities with the highest-correlation parameters and the (rescaled to [0, 1]) responses from Experiment 3 (conditional statements) are shown below.
...
A scatterplot of the same data is shown below.
...
### Fit and compared to Experiment 4: Relative clause
...
The model probabilities with the highest-correlation parameters and the (rescaled to [0, 1]) responses from Experiment 4 are shown below.
...
I should add confidence intervals for the model, but That Is Hard.
